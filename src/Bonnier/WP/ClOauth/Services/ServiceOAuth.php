...
class ServiceOAuth extends Client
{
    ...
    private $baseAuthorizationUrl = null;

    private $accessToken, $appId, $appSecret, $user, $userRoleList;

    public function __construct($appId, $appSecret, $baseAuthorizationUrl = null)
    {
        $this->appId = $appId;
    ...
    {
        if ($this->baseAuthorizationUrl !== self::SERVICE_URL) {
            $url = 'api/' . $url;
        }
        return $url;
    }
    ...
    {
        if (is_null($this->baseAuthorizationUrl)) {
